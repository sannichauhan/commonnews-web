...
class WPAlchemy_MetaBox {
     ...
     static function _get_current_post_type() {
         ...
         if (isset($uri)) {
             ...
             if ($uri AND in_array($file, array('post.php', 'post-new.php'))) {
                 ...
                 $post_type = $post_type === NULL && $post_id ? get_post_type($post_id) : $post_type;
    ...
    static function _get_post_id() {
        ...
        $post_id = isset($post->ID) && $post_id === NULL ? $post->ID : $post_id ;
    ...
    function can_output() {
        ...
        if
        (
                !empty($this->exclude_category) OR ! empty($this->exclude_category_id) OR ! empty($this->include_category) OR ! empty($this->include_category_id)
        ) {
        ...
        if
        (
                !empty($this->exclude_tag) OR ! empty($this->exclude_tag_id) OR ! empty($this->include_tag) OR ! empty($this->include_tag_id)
        ) {
